...
def distance_diff(series):
    # return [calculate_distance(series.iloc[i], series.iloc[i+1]) for i in range(len(series)-1)]
    return calculate_distance(series.iloc[0], series.iloc[1])
    # # 创建一个空列表来存储距离差
    # diff_list = []
    # # 计算相邻坐标点之间的距离差
    # for i in range(len(series) - 1):
    #     diff_list.append(calculate_distance(series.iloc[i], series.iloc[i+1]))
    # # 返回一个包含所有距离差的Series，而不是列表
    # return pd.Series(diff_list, index=series.index[1:])
